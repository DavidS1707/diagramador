    ...
    App.MainView = joint.mvc.View.extend({
        ...
        initializeStencil: function() {
            ...
            var stencil = this.stencil = new joint.ui.Stencil({
                ...
                dropAnimation: true,
                //para que aparezca la opcion Stencil donde abrimos o cerramos las categorias
            ...
            stencil.on('element:drop', function(elementView) {
                ...
                // Verificar si el elemento es de una de las clases específicas
                var allowedClasses = ['standard.Actor', 'standard.Entity', 'standard.Boundary', 'standard.Control', 'standard.Object'];
                if (allowedClasses.includes(elementView.model.get('type'))) {


                    var link = new joint.shapes.app.Link({
                        labels: [],
                        source: { id: elementView.model.id }, // Actor como fuente
                        target: { x: elementView.model.position().x + 40, y: elementView.model.position().y + 500 },// Punto de destino (abajo del actor)
                        id_objeto: elementView.model.id,
                        uml: 1
                        //  Vertices:[{x:elementView.model.position().x + 40,y:elementView.model.position().y + 250}],
                    });
                    console.log('LINEA DE VIDA:', link);
                    link.attr({
                        '.connection': { 'stroke-dasharray': '5,5', stroke: 'black' },
                    ...
                    elementView.model.set('lifeline', link.id);

                    console.log('OBJETO NUEVO:', elementView);

                    //guardaryrenderizardiagrama();
                    
                  //  setTimeout(function() {
                  //       VerificarBD =setInterval(verificarCambioBD, 1000);
                  //  }, 1000);
